...
//Funcion que muestra una alerta de que no se tienen permisos necesarios para realizar una accion
function alertaPermisos(){
    alert ("ACCIÓN NO REALIZADA, NO TIENES LOS PERMISOS NECESARIOS");
    return false;
}

//Funcion para realizar busqueda de un datos
function hacerPeticionAjax(valores){
    $.ajax({
        data: {valores},
        url: 'buscar.php',
        type: 'get',
...
function procesaRespuestaAjax(respuesta){
    ...
    for(i=0; i<respuesta.length; i++){
       res.push({'nombre_personaje': respuesta[i]['0'], 'id': respuesta[i]['1']});
    ...
    for(let i=0; i<res.length; i++){
        ...
        aux += "<p>" + res[i]['nombre_personaje'] + "</p>\n";
        nombren = res[i]['nombre_personaje'];
       console.log(nombren);
    }
